...
function M.render()
  if not (_ui.buf and vim.api.nvim_buf_is_valid(_ui.buf)) then
    return
  end
  _ui.help_open = false
  ...
  local win_width = vim.api.nvim_win_get_width(_ui.win)
  ...
  vim.api.nvim_buf_clear_namespace(_ui.buf, _ns, 0, -1)

  for i, range in ipairs(tab_ranges) do
    local hl = (i == _ui.cat_idx) and "DiagnosticVirtualTextInfo" or "TabLine"
    ui_utils.buf_hl(_ui.buf, _ns, hl, 1, range[1], range[2])
  end
  ui_utils.buf_hl(_ui.buf, _ns, "Comment", 1, hint_byte, hint_byte + #hint)
  ui_utils.buf_hl(_ui.buf, _ns, "Comment", 3, 0, -1)

  for lnum, entry in pairs(_ui.line_map) do
  ...
  for lnum in pairs(header_lnums) do
    ui_utils.buf_hl(_ui.buf, _ns, "Title", lnum - 1, 0, -1)
  end

  if _ui.win and vim.api.nvim_win_is_valid(_ui.win) then
    local wcfg = M.make_win_cfg()
    vim.api.nvim_win_set_config(_ui.win, {
      relative = "editor",
      height = wcfg.height,
      row = wcfg.row,
      col = wcfg.col,
  ...
  if _ui.win and vim.api.nvim_win_is_valid(_ui.win) then
    ...
    if first_entry_lnum then
      if cur < first_entry_lnum then
        vim.api.nvim_win_set_cursor(_ui.win, { first_entry_lnum, 0 })
      elseif cur > last_entry_lnum then
        vim.api.nvim_win_set_cursor(_ui.win, { last_entry_lnum, 0 })
      end
    end
  end
end
...
function M.toggle_help()
  _ui.help_open = not _ui.help_open
  if _ui.help_open then
    M.render_help()
  else
    M.render()
  end
...
function M.start_live_update()
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "LspDetach" }, {
    group = _ui.live_augroup,
    callback = function()
      vim.schedule(function()
        if
          _ui.win
          and vim.api.nvim_win_is_valid(_ui.win)
          and not _ui.help_open
        then
          M.render()
        end
      end)
...
function M.stop_live_update()
  if _ui.live_augroup then
    pcall(vim.api.nvim_del_augroup_by_id, _ui.live_augroup)
    _ui.live_augroup = nil
  end
end
